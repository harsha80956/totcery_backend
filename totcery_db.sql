...
alter table tot_user_address add constraint add_users_fk
foreign key (user_id) references tot_users(user_id);


create table tot_products(
...
alter table tot_products add constraint pro_users_fk
foreign key (user_id) references tot_users(user_id);
...
create sequence tot_customer_address_seq increment 1 start 1;
